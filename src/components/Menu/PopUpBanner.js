...
export default function PopUpBanner({overallwidth,leftholder,popbanner}) {
    ...
    const popupcalendarRef = useRef('')
    const setter = (val,valunik) => {
        setdayarrpopup(()=>val);
        setdayarrpopunik(()=>valunik)
       ...
       const clickday = (e)=>{
        ...
        if(dayarrpopup.includes(changingday)){
         ...
         unikarrbanner.splice(idx,1)
         newarr.splice(idx, 1);
         setter(newarr,unikarrbanner); 
        }
        else{
         setdayarrpopup(prev=> [...prev,changingday]);
         setdayarrpopunik(prev=> [...prev,identifier])
         settriggerlengthpop(prev=>prev+1)
       ...
       const tileClassName = (data) => {
        ...
        if (view === 'month') {
            if (dayarrpopup.includes(`${date.getDate()}-${date.getMonth()+1}-${date.getFullYear()}`)) {
  ...
  useEffect(()=>{
    if(popbanner && overallwidth>900){
      setordermarginTop(popupcalendarRef.current.offsetTop-leftholder.current.offsetTop)
    }
  },[popbanner])
  ...
  return (
    <div style={{padding:overallwidth>900?'20px':'20px 0px'}}>
      <p style={{width:'80%',margin:'10px auto',color:'#BABABA',textAlign:'left',fontFamily:'NexaTextLight',fontSize:'15px'}}>The pop-up banner will be displayed on loading of the home page.</p>
        <div style={{width:overallwidth>900?'50%':'85%',margin:'15px auto',border:'2px solid #0B1F36',borderRadius:'10px'}}>
          ...
          <p style={{width:'100%',textAlign:'center',padding:'10px',backgroundColor:'#02050a',borderRadius:'10px 10px 0px 0px',fontSize:'17px',boxSizing:'border-box'}}>Pop-Up Banner</p>
        ...
        <p ref={popupcalendarRef} style={{width:overallwidth>900?'50%':'70%',margin:'20px auto',textAlign:'center',marginBottom:'5px'}}>AVAILABLE SLOTS</p>
