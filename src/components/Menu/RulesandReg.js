...
export default function RulesandReg({overallwidth}) {
  ...
  return (
    <div className='rulesandregMain'>
            ...
            <div style={{display:overallwidth<700?'flex':'none',justifyContent:'space-around',width:'95%',margin:'20px auto'}}><p onClick={()=>{setheadertoshow(0)}} className= {headertoshow===0?'mobilebuttonrandr':'mobilebuttonrandrgrey'}>Terms & Conditions</p><p onClick={()=>{setheadertoshow(1)}} className= {headertoshow===1?'mobilebuttonrandr':'mobilebuttonrandrgrey'}>DYOR</p><p onClick={()=>{setheadertoshow(2)}} className= {headertoshow===2?'mobilebuttonrandr':'mobilebuttonrandrgrey'}>Privacy Policy</p></div>
