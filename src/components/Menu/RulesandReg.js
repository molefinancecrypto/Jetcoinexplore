...
export default function RulesandReg({overallwidth}) {
  ...
  return (
    <div className='rulesandregMain'>
            ...
            <div style={{display:overallwidth<700?'flex':'none',justifyContent:'space-around',width:'95%',margin:'20px auto'}}><p onClick={()=>{setheadertoshow(0);setheaderterms('Terms & Conditions')}} className= {headertoshow===0?'mobilebuttonrandr':'mobilebuttonrandrgrey'}>Terms & Conditions</p><p onClick={()=>{setheadertoshow(2);setheaderterms('Privacy Policy')}} className= {headertoshow===2?'mobilebuttonrandr':'mobilebuttonrandrgrey'}>Privacy Policy</p><p onClick={()=>{setheadertoshow(1);setheaderterms('DYOR')}} className= {headertoshow===1?'mobilebuttonrandr':'mobilebuttonrandrgrey'}>DYOR</p></div>
            <div style={{height:'auto',width:overallwidth<=900?'100%':'95%',margin:'20px auto',border:'1px solid #112836',backgroundImage: 'linear-gradient(90.18deg, #040B15 35.76%, rgba(6, 17, 31, 10) 110.28%)',borderRadius:'7px',padding:'20px',paddingTop:'0px',boxSizing:'border-box'}}>
                <div style={{display:overallwidth<=700?'none':'flex',position:'relative',border:'0.5px solid grey',borderWidth:'0px 0px 0.5px',paddingTop:'15px',paddingBottom:'15px',height:'40px',alignItems:'center',width:'100%'}}>
                  ...
                  <p style={{cursor:'pointer',textAlign:'center',width:'33.3%'}} onClick={ pcpolicy}>Privacy Policy</p>
